<x-dashboard-layout>
    <x-slot name="header">
        {{ __('Pengajuan Surat Rekomendasi Izin Praktik untuk Non-Anggota') }}
    </x-slot>
        ...
        <div class="bg-indigo-300 p-2 text-black text-2xl font-bold text-center mb-4 rounded">
            ...
            <form action="{{ route('submission.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        <x-input-label for="scan_surat_pengantar" :value="__('Scan Surat Pengantar dari Cabang Asal')" :required=true />
                        <input type="file" name="scan_surat_pengantar" class="w-full px-4 py-2 border border-gray-300 rounded" required>
                        <span class= "text-gray-500 text-sm px-1 inline-block">(maksimal ukuran file: 1024 KB, format file: png, jpg, jpeg, atau pdf)</span>
                        <x-input-error :messages="$errors->get('scan_surat_pengantar')" class="mt-2" />
                    </div>
                    <div>
                        <x-input-label for="scan_surat_kolegium" :value="__('Scan Surat Kolegium')"  :opsional=true />
