                    ...
                    <tr class="divide-x divide-gray-200">
                            ...
                            <td class="whitespace-nowrap py-4 pl-4 pr-4 text-sm font-medium text-gray-900 sm:pl-6">
                                <form action="{{ route('zoom_meetings.status', $zoomMeeting) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
                                    <select class="border border-gray-300" name="status" onchange="this.form.submit()">
                                        <option value="Hadir" {{ $zoomMeeting->status === 'Hadir' ? 'selected' : '' }}>Hadir</option>
                                        <option value="Tidak Hadir" {{ $zoomMeeting->status === 'Tidak Hadir' ? 'selected' : '' }}>Tidak Hadir</option>
                                    </select>
                                </form>
                            </td>
                        </tr>
